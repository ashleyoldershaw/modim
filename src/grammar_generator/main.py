...
def generate(topic_name, topic, topic_lang, lang):
...
# Here we create the GRAMMAR file
    ...
    with open('topics/'+topic_name+'.txt', 'r', encoding="utf-8") as f:
        for line in f:
            l, r = line.rstrip('\r\n').split('->')
            r = r.strip()
            for m in l.split(','):
    ...
    write_xml('grammars/'+topic_lang+'.grxml', doc) 
    ...
    write_xml('grammars/frame_'+topic_lang+'.grxml', doc) 
    ...
    write_xml('grammars/'+topic+'.xml', doc)
